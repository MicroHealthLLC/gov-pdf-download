...
class GAODoDReportScraper:
    def __init__(self, output_folder="gao_dod_reports"):
        """Initialize the DoD-specific scraper with output folder"""
    ...
    def build_gao_dod_url(self, start_date_str, page_num=0):
        """Build GAO URL with DoD agency filters and dynamic date range"""
        try:
            ...
            # Build the base URL with agency filters
            url = f"{self.base_url}/reports-testimonies?"
            
            # Add agency filters (f[0], f[1], f[2], f[3])
            for idx, agency in enumerate(self.dod_agencies):
                encoded_agency = agency.replace(" ", "%20")
                url += f"f%5B{idx}%5D=by_agency_name%3A{encoded_agency}&"
            
            # Add date range filter (f[4])
            url += f"f%5B{len(self.dod_agencies)}%5D=rt_date_range_gui%3A%28min%3A{start_timestamp}%2Cmax%3A{end_timestamp}%29"
    ...
    async def scrape_reports(self, start_date_str):
        """Main scraping function with pagination support for DoD reports"""
        
        async with async_playwright() as p:
            # Launch browser with enhanced stealth settings
            browser = await p.chromium.launch(
                headless=False,  # Can use headless with stealth config
                args=[
            ...
            # Create context with comprehensive fingerprint spoofing
            ...
            listing_url, start_date, end_date = self.build_gao_dod_url(start_date_str, 0)
            
            print(f"\n{'='*70}")
            print(f"GAO DoD Report Scraper")
            print(f"{'='*70}")
            print(f"Agencies: {', '.join(self.dod_agencies)}")
            ...
            for page_num in range(total_pages + 1):
                listing_url, _, _ = self.build_gao_dod_url(start_date_str, page_num)
...
def get_start_date_from_user():
    ...
    print("\n" + "="*70)
    print("GAO DoD Report Scraper - Interactive Mode")
    print("="*70)
    print("\nThis tool will download all GAO reports for Department of Defense")
    print("agencies from your specified start date through today.\n")
    print("Agencies included:")
    print("  • Department of Defense")
    print("  • Department of the Air Force")
    print("  • Department of the Army")
    print("  • Department of the Navy\n")
    
    while True:
        start_date_input = input("Enter start date (YYYY-MM-DD, e.g., 2024-01-31): ").strip()
        
        date_pattern = r'^\d{4}-\d{2}-\d{2}$'
        if not re.match(date_pattern, start_date_input):
            print("❌ Invalid format. Please use YYYY-MM-DD format (e.g., 2024-01-31)\n")
            continue
        ...
        try:
            ...
            print("❌ Invalid date. Please enter a valid date (e.g., 2024-01-31)\n")

def main():
    start_date_str = get_start_date_from_user()
    end_date_str = datetime.now().strftime("%Y-%m-%d")
    output_folder = f"GAO-DoD-{start_date_str}-{end_date_str}"
    
    print(f"\n✓ Start date set to: {start_date_str}")
    print(f"✓ End date (today): {end_date_str}")
    print(f"✓ Output folder: {output_folder}\n")
    
    scraper = GAODoDReportScraper(output_folder=output_folder)
