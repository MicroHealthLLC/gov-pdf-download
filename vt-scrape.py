...
class VTechWorksBookScraper:
    ...
    async def get_books_from_page(self, page, page_num):
        ...
        try:
            ...
            # Strategy 1: Look for any links containing /items/
            ...
            for link in all_links:
                try:
                    ...
                    if href and '/items/' in href:
                        ...
                        # Only add if it's a direct item link (not a collection or search)
            ...
            print(f"  Found {len(book_list)} unique book links on page {page_num}")
            
            # Print first few for verification
            if book_list:
                print(f"  Sample links:")
                for i, link in enumerate(book_list[:3], 1):
                    print(f"    {i}. {link}")
    ...
    async def get_all_book_links(self, page):
        ...
        for page_num in range(1, 4):  # Pages 1, 2, 3
            ...
            await asyncio.sleep(2)  # Be polite to server
    ...
    async def extract_pdf_download_link(self, page, book_url):
        ...
        try:
            ...
            # Find download links - look for bitstreams
            download_links = await page.locator('a[href*="/bitstreams/"]').all()
            
            print(f"  Found {len(download_links)} bitstream links")
            
            for link in download_links:
                try:
                    href = await link.get_attribute('href')
                    
                    # Get parent element text to check file info
                    parent = link.locator('xpath=..')
                    parent_text = await parent.text_content() if await parent.count() > 0 else ""
                    
                    # Also get the link text itself
                    ...
                    if '.pdf' in combined_text and '/download' in href:
                        ...
                        print(f"  ✓ Found PDF: {title} ({file_size})")
            ...
            print(f"  ⚠ No suitable PDF found for: {title}")
            return None
    ...
    async def run(self):
        ...
        async with async_playwright() as p:
            ...
            print("=" * 70)
            print("VTechWorks Open Textbooks Scraper (Debug Version)")
            print("=" * 70)
            ...
            if len(book_links) == 0:
                ...
                print("Check the debug_page_*.png screenshots to see what the page looks like.")
                await browser.close()
                return
